...
Func Main()
	; we are in the outpost, wait until we are in uw
	...
	Do
		Sleep(200)
		Local $lMapLoading = GetMapLoading()
		If TimerDiff($lDeadlock) > 8*1000 And GetMapLoading()==$INSTANCETYPE_EXPLORABLE Then
			Resign()
			Sleep(3000)
			If DllStructGetData(GetAgentByID(-2), 'PlayerNumber') == 1 Then ReturnToOutpost()
			$lDeadlock = TimerInit()
		EndIf
	Until $lMapLoading <> 2 And GetMapIsLoaded() And GetMapID() == $USED_CITY
	RndSleep(1500)
...
Func SpikeSkele($SkeleID, $CallerID)
	...
	Do
		Sleep(100)
	Until GetIsDead($SkeleID) Or GetIsDead(-2) Or GetIsDead($CallerID)
EndFunc
